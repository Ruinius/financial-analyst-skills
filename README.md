...
These are actual screenshots of the final, static artifacts generated by the financial-analyst-skills pipeline.

<div align="center">
  <table>
    <tr>
      <td align="center">
        <kbd><img src="assets/markdown_preview.png" width="800" alt="Structured Markdown Output"></kbd><br>
        <em>Screenshot: The rendered master markdown file containing all extracted financial data and qualitative assessments.</em>
      </td>
      <td align="center">
        <kbd><img src="assets/viewer_preview.png" width="800" alt="Interactive DCF Model Viewer"></kbd><br>
        <em>Screenshot: The zero-dependency HTML viewer rendering the final valuation model.</em>
      </td>
    </tr>
  </table>
</div>
...
```
PDF → Classification → Extraction → Calculations → Organization → DCF Model → JSON Export → Interactive Viewer
```

## Skills

All 8 skills are registered in [`skills/skills_metadata.json`](skills/skills_metadata.json). Each skill is a single, consolidated `SKILL.md` with step-by-step instructions, worked examples, validation checks, and expected output formats. A shared [`SHARED_POSTRUN.md`](skills/SHARED_POSTRUN.md) defines common example-curation and self-improvement steps.
...
| 6     | [**Financial Modeling**](skills/financial_modeling/)               | Single script: WACC → three-stage assumptions → 10-year DCF → intrinsic value per share                       |
| 7     | [**Model JSON Generator**](skills/model_json_generator/)           | Formats the metadata markdown into a clean, structured JSON file for the interactive DCF viewer                |
...
│   │   ├── SKILL.md            # Moat + Margin + Growth trajectory assessment
│   │   ├── examples/
│   │   └── scripts/            # assess.py
│   ├── financial_modeling/
│   │   ├── SKILL.md            # WACC + 3-stage assumptions + DCF + Intrinsic Value
│   │   ├── examples/
│   │   └── scripts/            # calculate.py
│   ├── model_json_generator/
│   │   ├── SKILL.md            # JSON formatting for interactive viewer
│   │   ├── examples/           # ADBE_financial_model.json reference
│   │   └── scripts/            # generate_json.py
├── tools/                      # Shared tools and utilities
│   ├── market_data.py          # Yahoo Finance integration
│   ├── tiger_transformer_server.py
│   ├── simple_frontend_server.py
│   └── financial_model_viewer.html
